...
public class SpawningManagement : MonoBehaviour
{
    ...
    */
    ...
    ///     ForceUpdate will force the method call to stop ALL Spawn Coroutines, recalculate difficulty and start spawnning
    ///     again
    ///     It is typically used for development purposes.
    /// </param>
    /// <returns></returns>
        ...
        while (true)
        {
            ...
            */
            if (Factor != currentFactor || forceUpdate)
            {
                ...
                var asteroidSpawn = Mathf.Clamp(asteroidSpawnRate - Factor, maxDifficultyAsteroidSpawnRate,
                    asteroidSpawnRate);
    ...
    /// <param name="minimumDistanceAway">
    ///     This refers to the minimum distance away from the Relative Transform the object
    ///     should spawn in
    /// </param>
    /// <param name="maximumDistanceAway">
    ///     This refers to the maximum distance away from the Relative Transform the object
    ///     should spawn in
    /// </param>
    /// <returns></returns>
    private IEnumerator Spawner(GameObject[] objectsToSpawn, Transform relativeTo, float spawnRate,
        float minimumDistanceAway, float maximumDistanceAway)
    {
        while (true)
        {
